...
`.trio/routing.json`에서 변경 가능:

```json
{
  "rules": {
    "architecture": "claude-opus-4.6",
    "implementation": "gpt-5.4-codex",
...
### 🔄 Flow — 워크플로우 흐름도
Phase별로 태스크가 어떤 모델에서 병렬/순차로 실행되는지 실시간으로 보여준다. 에이전트별 색상으로 구분.

### 📄 Results — 최종 결과
모든 태스크 output을 합쳐서 한 화면에 보여준다. Gemini가 AI 요약을 자동 생성한다. 원본 출력은 접기로 확인 가능.

### 📋 Tasks — 칸반 보드
태스크를 Pending → Working → Done / Error 상태로 관리. 드래그앤드롭 지원.

### ⚙️ Setup — 설정
CLI 설치/인증, 모델 라우팅 변경, 워크플로우 설정 (어떤 태스크를 켜고 끌지), 최신 모델 확인.
...
그게 전부다. CLI 도구 (Claude Code, Codex, Gemini CLI)는 대시보드 Setup 탭에서 설치할 수 있다.
